  ...
  transition('* <=> *', [
    ...
    /* 2 */ group([  // block executes in parallel
      query(':enter #aboutContainer', [
        ...
        animate('0.65s ease-out', style({ transform: 'translateX(0px)', position: 'relative' }))
      ], { optional: true }),

      query(':enter #portfolioContainer', [
        style({ transform: 'translateX(100%)' }),
        animate('.65s ease-out', style({ transform: 'translateX(0px)', position: 'relative'}))
        ], {optional: true}),

      query(':leave #portfolioContainer, :leave #cuttleContainer, :leave #assistmentsContainer', [
        style({ transform: 'translateX(0%)' }),
        animate('0.5s ease-in-out', style({ transform: 'translateX(175%)', position: 'relative' }))
      ], { optional: true }),
